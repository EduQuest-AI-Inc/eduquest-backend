-- Migration 014: add SECURITY DEFINER to fork_marketplace_listing so that the
-- internal SELECT ... FROM period (and week/lesson/etc.) runs with owner privileges
-- and is not blocked by RLS on the authenticated role.
-- Without this, the INSERT ... SELECT inserts 0 rows silently when the calling
-- user cannot read the original period, and the fork appears to succeed (204)
-- but creates no data.
...
BEGIN
  ...
  IF NOT FOUND THEN
    RAISE EXCEPTION 'original_period_not_found';
  END IF;
