"""Tests for OrchestratorAgent. Lives in unit/bots/ to use real_bots_imports fixture."""
    ...
    return CompleteSlideDeck(
        ...
        slides=[
            ...
            CompletedSlide(
                ...
                title="Glycolysis",
                bullets=["Glucose → 2 pyruvate.", "Occurs in cytoplasm", "Yields 2 ATP"],
...
@pytest.mark.unit
def test_orchestrator_returns_complete_deck():
    ...
    deck = asyncio.run(OrchestratorAgent().run_async(SAMPLE_LESSON, SAMPLE_CONTEXT))
...
def test_skill_card_has_bloom_and_difficulty():
    ...
    for slide in skill_slides:
        assert slide.bloom_level
        assert slide.difficulty
