...
class TestInitiateLTGConversation:
    ...
    def test_initiate_ltg_success(self, client):
        with patch("routers.period.period_service") as mock_ps:
...
class TestAcceptParentInvite:
    ...
    def test_accept_invite_success(self, client):
        with patch("routers.period.parent_service_p") as mock_parent:
...
class TestCreatePeriod:
    ...
    def test_create_period_teacher_with_pilot_access_returns_201(self, client):
        mock_vs = MagicMock()
        mock_vs.id = "vs-1"
        with patch("routers.period.teacher_dao_p") as mock_td, \
             patch("routers.period.append_canvas_file"), \
             patch("routers.period.create_vector_store") as mock_cvs, \
             patch("routers.period.upload_period_files", return_value=[]) as _mock_up, \
             patch("routers.period.try_generate_schedule", return_value={"schedule": {}}) as _mock_sched, \
    ...
    def test_create_period_teacher_without_pilot_access_returns_403(self, client):
        with patch.dict(os.environ, {"PILOT_WAITLIST_ENABLED": "true"}), \
             patch("routers.period.teacher_dao_p") as mock_td, \
             patch("routers.period.waitlist_service_p") as mock_wl:
...
class TestAddFilesToPeriod:
    ...
    def test_add_files_success(self, client):
        with patch("routers.period.period_management_service") as mock_pms, \
             patch("routers.period.upload_period_files",
    ...
    def test_add_files_service_error_returns_500(self, client):
        with patch("routers.period.period_management_service") as mock_pms, \
             patch("routers.period.upload_period_files") as mock_up:
            mock_pms.period_dao.get_period_by_id.return_value = OWNED_PERIOD
...
class TestUnenrollRoute:
    ...
    @pytest.mark.api
    @patch("routers.period.period_service")
